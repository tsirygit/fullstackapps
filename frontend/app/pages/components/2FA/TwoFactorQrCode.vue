<template>
  <div class="bg-white p-4 border rounded-xl shadow-sm text-center">
    ...
    <div class="mt-6">
      <p class="mb-2 font-semibold">Entrez le code Google Authenticator</p>
  ...
  <div v-if="recoveryCode.length > 0" class="mt-4">
    <h3 class="font-bold mb-2">Recovery Codes</h3>

    <ul class="bg-gray-100 p-4 rounded-lg">
      <li v-for="item in recoveryCode" :key="item" class="text-sm">
        {{ item }}
      </li>
    </ul>
  </div>
